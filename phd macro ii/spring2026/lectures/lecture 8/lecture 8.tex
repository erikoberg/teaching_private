...
\bit
	...
	\item Search models offer a theory of unemployment and wage dispersion
	
	\item Today we dig deeper into the determinants of wage dispersion

	\item The McCall model: search frictions + offer distribution $\Rightarrow$ reservation wage strategy

	\item It generates a sensible theory of residual wage dispersion

	
	\item But the model had no chance of matching empirical measures of wage dispersion
	
	\item And how can exogenous wage dispersion be consistent with firm wage posting in the first place (Rothschild critique/Diamond paradox)?
\eit
...
\item Today we introduce a class of models that speaks to both of these problems
\bit
...
\item Generates more residual wage dispersion because ex ante identical workers differ not only because their initial offers differ, but also because some have climbed further up the job ladder than others
...
\section{The AKM Regression}
\sectiondivider{The AKM regression}
...
\item Abowd, Kramarz, and Margolis (Econometrica, 1999) were the first to use French matched employer-employee data to estimate
\begin{eqnarray}
\log y_{it} = \mu_y + \beta \mathbf{X}_{it} + \theta_i + \Psi_{j(i,t)} +  \epsilon_{it} \nonumber
\end{eqnarray}
for individual $i$ working for firm $j(i,t)$ at time $t$

\item A reduced-form empirical framework for studying labor market sorting and inequality
\bit
 	\item Card-Heining-Kline (QJE 2013); Bloom-Guvenen-Price-Song-von Wachter (QJE 2019); Frederiksen-Hensvik-Skans (AER 2018); Eliason-Hensvik-Kramarz-Skans (JE 2023)
\eit

\item $\theta_i$ is identified from variation in worker earnings within firms; $\Psi_{j(i,t)}$ is identified from changes in worker earnings when workers switch jobs
...
\bit
	...
	\item Some workers are persistently paid more across all their jobs than other, otherwise similar, workers
	
	\item Some firms persistently pay more to all their employees, even though those employees are similar to workers at other firms
...
\section{The Burdett-Mortensen Model}
\sectiondivider{The Burdett-Mortensen model}
...
\item Firms decide which wage $w$ to post, generating the offer distribution $F(w)$
%\bit
...
\item Each worker's problem is to accept or reject offers
...
\item Characterizing worker behavior boils down to finding $w_R$, given $F$, $b$, $\lambda_e$, $\lambda_u$, and $\sigma$

\item For now, we assume that $F$ has bounded support; later we prove that this follows from firm optimization
...
\item $\lambda_u>\lambda_e \Rightarrow w_R>b$: workers find better opportunities while unemployed $\Rightarrow$ unemployed workers demand compensation for giving up the option value of search

\item $\lambda_u<\lambda_e \Rightarrow w_R<b$: workers find better opportunities while employed $\Rightarrow$ unemployed workers accept a wage below $b$ in anticipation that they will receive better offers more quickly once employed
\eit
%
...
\item Rewrite the employed worker's value:
\begin{eqnarray}
...
\item We have now also shown that $W(w)$ is increasing in $w$
...
\item We have now characterized worker behavior, taking the offer distribution $F(w)$ as given

\item Before turning to the firm problem, which determines $F(w)$, we derive the worker distribution $G(w)$, taking $F(w)$ as given
...
\bit
	...
	\item outflow 2: employed households with wage $\leq w$ that find a job with wage $>w$
\eit
...
\bit
	\item Example: assume the offer distribution $F(w)$ is normal, compute $w_R$ from \eqref{reservation_wage_final}, and then compute $G(w)$ from \eqref{worker_distribution}
...
\item Given $G$ and $F$, we can also characterize the firm size distribution

\item Use a limit argument: determine how many firms offer jobs and how many workers hold jobs on the interval $[w-\epsilon,w]$, then let $\epsilon \rightarrow 0$

\item Workers on interval $[w-\epsilon,w]$: $\left[G(w)-G(w-\epsilon)\right]\times (1-u)$

\item Firms on interval $[w-\epsilon,w]$: $\left[F(w)-F(w-\epsilon)\right] \times 1$
...
\item Given our solution for $u$ and $G$, we can solve for $I$
...
\item Firms maximize the expected discounted sum of profits
...
\item Why don't all firms set $w=w_R$ (as in the wage-posting equilibrium of the McCall model)?
...
\item Given the progress we have made on characterizing worker behavior and the firm size distribution, the equilibrium can be solved in five steps:
\ben
\setlength\itemsep{1em}
...
\item No worker accepts a wage $w < w_R \Rightarrow \pi(w<w_R) = 0$

\item Also, $\pi(w>y) < 0$

\item Therefore, $F$ has bounded support $[\underbar w, \bar w] \subset [w_R, y]$
...
\item Proof strategy: if $F$ has a mass point at $\hat w$, a firm can make excess profits by offering $\hat w + \epsilon$
...
\item A firm can make excess profits by offering wage $\hat{w}+\epsilon$ because
\bit
	\item per-worker profit decreases continuously
	\item firm competition decreases discretely
\eit
...
\item An employed worker with wage $w$ switches jobs at rate $\lambda_e(1-F(w))$

\item Total job switch rate: $ \xi = \int_{w_R}^{\bar w} \lambda_e (1-F(w)) d G(w)$


\item One can show that, in equilibrium, $\xi = \xi(\sigma, \lambda_e)$ (take-home exercise)


\item $\lambda_u$ is directly identified from the job-finding rate of unemployed workers, since unemployed workers accept all offers in this model
...
\begin{frame}{Predictions I: wage dispersion}
...
\item The BM model generates additional dispersion, as workers differ not only in their initial offers, but also in how far they have climbed up the job ladder

\item Hornstein-Krusell-Violante (AER, 2011) report a monthly EE rate of 3.2 percent $\Rightarrow \lambda_e = 0.135$

\item {\bc$Mm_{BM} = 1.22$}, $Mm_{McCall} = 1.05$, $Mm_{data} > 1.8$

\item Still not there, but a big step in the right direction
\bit
...
\begin{frame}{Predictions II: wage correlations}

\bit
\setlength\itemsep{2em}

\item A worker with wage $w$ finds a new job at rate:
\begin{eqnarray}
\lb{ee_rate}
...
\item This challenges the interpretation of returns to tenure as reflecting productivity

\item Firm size is positively correlated with wage
\bit
	\item From \eqref{firm_size}: $I(w)$ is increasing in $w$, because workers continuously climb the job ladder
\eit

\item Both are true in the data (Mortensen, Book 2003)
%
...
\item We come closer to accounting quantitatively for residual wage dispersion

\item BM also offers an interpretation of the AKM regression results
	\bit
	\setlength\itemsep{0.5em}
		\item all wage dispersion stems from firm heterogeneity, even though all firms are identical in terms of productivity
...
\bit
\setlength\itemsep{0.5em}
	\item within-firm wage dispersion and, therefore, the individual fixed effects in AKM
...
\item The basic BM model is too stylized to account for many empirical phenomena
...
\item For quantitative assessments, see Jolivet-Postel-Vinay-Robin (EER 2006), Mortensen (Book 2003), and Barlevy (ReStud 2008)
...
\item Gottfries-Jarosch (2023) estimate a BM model for the US to quantify how much non-compete practices can suppress wages
\bit
\setlength\itemsep{0.5em}
\item A BM model with a finite number of firms and DRS production technology provides a natural laboratory for studying monopsony power

\item ``Non-competes'' are modeled as wage offers that do not allow job-to-job transitions
...
\item BM theory: because firms set wage offers strategically in relation to $F(w)$, a higher minimum wage affects the entire distribution

\item They find that 70 percent of the drop in earnings inequality during this period can be attributed to the higher mandated minimum wage
\eit
\eit
...
\begin{center}
	\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{figures/em_fig1.pdf}
\end{center}
...
\item Key question: is wage posting a reasonable benchmark for the applications considered?
\bit	
\setlength\itemsep{0.5em}
...
\bit
	\item Stevens (ReStud 2004) and Burdett-Coles (Econometrica, 2003) note that firms can reduce turnover and raise profits by offering wage-tenure contracts
\eit

\item Shouldn't firms be allowed to respond to employees' outside offers?
\eit

\item Postel-Vinay and Robin (Econometrica, 2002) construct a job-ladder model extended with
\bit
	\setlength\itemsep{0.5em}
	\item firm and worker heterogeneity
	\item allowing firms to make counteroffers
	\item rationalizing within-firm wage dispersion among similar workers as well as downward wage mobility
	\item offering a way to quantitatively separate the role of unobserved firm heterogeneity, unobserved worker heterogeneity, and luck in residual wage dispersion
\eit
\eit
...
\item Key result: a non-degenerate distribution of wages across identical firms due to the trade-off in the wage-posting decision
\bit
\item Higher wage attracts more workers
\item Higher wage reduces per-worker profit
\eit
...
\item Next up: endogenizing the contact rate (and thinking about its implications for unemployment)
