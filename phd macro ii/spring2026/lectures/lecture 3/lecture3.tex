...
\item $\mathbb E M_{t,t+s}$ prices \emph{risk-free} assets that pays off in period $t+s$
...
\bit
\setlength\itemsep{1.5em} 
\item Set up the Lagrangian with mulitplier $q_t$, take the F.O.C. to find: {\rc (Do on whiteboard)}
\begin{eqnarray}
W_t &=& A_t F_N(K_t, N_t)  \\
q_t &=& 1  \\
...
\item Note: Equation (4) is also an optimality condition for the household. Suppose households are optimizing, what does this mean for the choice of $B_{t+1}$ of the firm {\bc Miller-Modgliani (AER 1963): the capital structure of the firm is irrelevant if the household can trade in the same assets as the firm}
...
N_t: \hspace{3mm} W_t &=& A_t F_N(K_t, N_t)  \\
I_{t}: \hspace{3mm} q_t &=& 1  \\
K_{t+1}: \hspace{3mm} q_t &=&  \mathbb E_t \left[M_{t,t+1}\left[A_{t+1}F_K(K_{t+1}, N_{t+1}) + (1-\delta)q_{t+1} \right] \right]  \\
B_{t+1}: \hspace{3mm} Q_t &=& \mathbb E_t M_{t,t+1} 
\end{eqnarray}
where $q_t$ is the Lagrange multiplier on the firm constraint
...
\begin{frame}{How to incorporate the Q theory of investment in our model?}
...
\item News shocks: see, e.g., Beaudry-Portier (AER 2006; Koslyk 2025)
\bit
