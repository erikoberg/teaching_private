...
\item In contrast to RBC, NK predicts inefficient fluctuations, and a role for monetary policy in affecting the equilibrium

\item Today: when and how should monetary policy be used to address fluctuations in the NK model?
\eit
...
\item Inefficiencies in the NK model
...
\begin{center}
	\huge Inefficiencies in the NK model \normalfont
\end{center}

\end{frame}


\begin{frame}{Inefficiencies in the NK model}
...
\item Set up the Lagrangian and take the F.O.C.s; you will find:
\begin{eqnarray}
...
\begin{frame}{Inefficiencies with flexible prices}
...
\item This is a \emph{static} distortion; it has nothing to do with the dynamic response to shocks

\item Going forward, we eliminate the static distortion with a lump-sum-financed firm subsidy $(1-\tau)$, implying the firm F.O.C.
\begin{eqnarray}
...
\item Calvo pricing implies some firms cannot update prices, and the ones that do update do not set $P_{it} = M \cdot MC^{nom}_t$ $\Rightarrow$ fluctuations in $M_t$
\bit
...
\item Price dispersion generates an inefficiency due to {\bc misallocation}
...
\item Then there is no price dispersion and no time-varying markup $\Rightarrow$ the first best is attained

\item $\Rightarrow$ the socially optimal allocation is achieved if all price setters keep the initial price

\item Could (in theory) be implemented with a time-varying subsidy to resetters

\item Could also (in theory) be implemented by time-varying taxes on household financial decisions $\Rightarrow$ inefficiency can be conceptualized as arising from an {\bc aggregate-demand externality} (Farhi-Werning, Econometrica 2016; see also Correia-Farhi-Nicolini-Teles, AER 2013)
...
\bit
\setlength\itemsep{0.5em}
\item Zero inflation, not for its own sake, but because it eliminates markup fluctuations and price dispersion
...
\begin{frame}{How can monetary policy implement the first best? Part I}
...
\item Ergo, this rule does not guarantee that the first best is achieved
\eit

\end{frame}

\begin{frame}{How can monetary policy implement the first best? Part II}
...
\item Same determinacy condition as before: $\phi>1$ achieves determinacy, so $\pi_t = 0$, $\tilde y_t=0$ is the unique bounded solution
...
\bit
\setlength\itemsep{0.5em}
	\item However, in theory, simple policy rules, e.g., $\hat i_t = \phi \pi_t$, effectively mitigate welfare losses if $\phi$ is sufficiently high
\eit

\item TFP shocks do not generate a trade-off for monetary policy decisions
\bit
\setlength\itemsep{0.5em}
\item If you stabilize inflation, you also stabilize the output gap

\item Sometimes called the {\bc divine coincidence}

\item No rationale for a dual mandate
\eit
...
\item With cost-push shocks, we need a bit more machinery to get at optimal policy

\item In general, the policy problem is to set a sequence $\{\hat i_t \}$ to maximize some social objective function, subject to the allocation being an equilibrium:
\begin{eqnarray}
...
\item To put the welfare function on par with the equilibrium expressed in terms of a log-linear approximation around steady state, we need to approximate it
...
\item Note: this result stems from the fact that the equilibrium has no state variable
...
\item Optimal policy features a choice between an ``inefficient'' recession and higher inflation
...
\item The trade-off between output and inflation is still qualitatively similar, but now optimal policy features some degree of smoothing: a high $x_{t-1}$ implies a high value of $x_t$
...
\begin{frame}{Discretion vs. commitment, a surprising result}

\bit
\setlength\itemsep{1.5em}

\item Under both discretion and commitment, the central bank is benevolent and has social welfare in mind

\item The difference between the two policies is {\bc not} information or intentions, but {\bc credibility}
\bit
\setlength\itemsep{0.5em}
\item Under discretion, the central bank re-optimizes every period and takes future behavior as given

\item Under commitment, it can shape private-sector expectations by promising future actions that may be ex post unattractive, but ex ante welfare improving
\eit

\item This is the core time-inconsistency insight: even a well-intentioned policymaker may want institutions or rules that tie its own hands
\bit
\setlength\itemsep{0.5em}
	\item See Kydland-Prescott (JPE 1977) 
\eit
...
\begin{frame}{Intermezzo: From time inconsistency to political economy}

\bit
\setlength\itemsep{1.5em}

\item Kydland-Prescott changed how economists think about policy: from an optimal-control problem to a {\bc strategic interaction} between policymakers and private agents

\item Once credibility becomes central, institutions become central too: rules, delegation, reputation, and central-bank independence
\bit
	\item Kydland-Prescott (JPE 1977) was important for the establishment of central-bank independence
\eit

\item The time-inconsistency insight was central to birth of modern {\bc political economy}
\bit
\setlength\itemsep{0.5em}
\item policy is shaped not only by technology and preferences, but also by incentives, commitment problems, and political institutions

\item See Tabellini (SJE 2005) for a beautiful celebration of the Kydland-Prescott insight
\eit
\eit
...
\item Research program initiated by Christiano-Eichenbaum-Evans (JPE 2005) and Smets-Wouters (JEDC 2003; AER 2007)

\item Quantitative NK models are used by many central banks to make forecasts and analyze policy interventions
\bit
...
\setlength\itemsep{0.5em}
	...
	\item Helps explain some asset-pricing puzzles (see, e.g., Campbell-Cochrane, JPE 1999)
	...
	\item Alternative: sticky expectations (Mankiw-Reis, QJE 2002)
\eit
\eit
...
\bit
\setlength\itemsep{1.5em}
\item The data are indicative of wage rigidities
\bit
\setlength\itemsep{0.5em}
\item The average wage rate is not very volatile, although hours worked and productivity are

\item Micro data suggest that some of the low aggregate volatility is due to selection, and some is due to rigidity
\eit

\item Wage rigidity can be modeled analogously to Calvo-style price rigidity (Erceg-Henderson-Levin, JME 2000)
\bit
\setlength\itemsep{0.5em}
...
\item A good starting point, but based on assumptions that are not entirely compelling

\item Broer-Harmenberg-Krusell-\"{O}berg (AER: Insights 2023), Harmenberg-\"{O}berg-Olsson (2026): frameworks for modeling wage rigidity consistent with competitive/monoposonistic labor markets
\eit
...
\bit
\setlength\itemsep{1.5em}
\item Christiano-Eichenbaum-Evans: set parameter values to minimize the distance between the model IRF and the data IRF for monetary policy shocks
\ben
\setlength\itemsep{0.5em}
...
\setlength\itemsep{0.5em}
	...
	\item This means specifying a prior distribution of parameter values, and then updating according to Bayes' law when the model is confronted with data
\eit
...
\item CEE show that rigid wages are the key friction needed to match these moments
\eit
...
\item When the flexible-price equilibrium is optimal, policy can mitigate efficiency losses, in terms of both inflation and output, by implementing the natural real interest rate

\item When shocks distort the flexible-price equilibrium, policy faces a trade-off between stabilizing inflation and output, as well as a time-inconsistency problem

\item With a reasonable set of model extensions, the NK model can match empirical IRFs to monetary policy shocks in quite some detail
...
\bit
	\item Hicks, Samuelson, Modigliani, Tobin, etc.
\eit
...
\item 1980s: theoretical insights regarding the interplay of ``aggregate demand'', price stickiness, and monopoly power
\bit
\setlength\itemsep{0.5em} 
