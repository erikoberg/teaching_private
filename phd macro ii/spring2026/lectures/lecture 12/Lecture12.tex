...
\title[The Buffer-Stock Savings Model]{\large Macroeconomics II, Lecture XII:\\
...
\item Three influential applications
\ben
\setlength\itemsep{0.5em}
	\item Gourinchas-Parker (Econometrica 2002): Consumption over the Life Cycle

	\item Blundell-Pistaferri-Preston (AER 2008): Consumption Inequality and Partial Insurance

	\item Kaplan-Violante (Econometrica 2014): A Model of the Consumption Response to Fiscal Stimulus Payments
\een
...
\item The buffer-stock savings model $=$ income-fluctuations problem with persistent shocks
\bit
	\item Key papers: Zeldes (JPE 1989; QJE 1989), Deaton (Econometrica 1991, book 1992), Carroll (QJE 1997)
\eit
...
\begin{frame}{The canonical buffer-stock savings model: terminology}
...
\item The permanent-transitory formulation provides a parsimonious parameterization of the household income process that captures these features
\eit

\item CRRA utility: a reasonable baseline with large tractability gains
\bit
\item Allows us to normalize the problem with respect to permanent income
\eit
...
\item To solve the problem and investigate its properties, we need to recast it in recursive form
...
\bit
\item What information, known at time $t$, is useful for the household in choosing $C, A'$ and computing $U(C)+\beta E V'()$?
\eit
...
\begin{frame}{Normalization with respect to permanent income}
...
\begin{frame}{Normalization with respect to permanent income II}
...
\begin{frame}{Normalization with respect to permanent income III}
...
\setlength\itemsep{0.5em}
	...
	\item Unnormalized decision functions $C=Pc(m)$, $A'=Pa'(m)$
	
	\item Implication: a permanent-income rich household will consume the same fraction of an increase in normalized cash-on-hand as a permanent-income poor household
\eit
...
\bit
\setlength\itemsep{0.5em}
\item We have reduced a two-dimensional functional equation to a one-dimensional functional equation
...
%\bit
%\setlength\itemsep{0.5em}
%	\item Impatience condition: sufficiently low $\beta R$ to ensure that assets converge asymptotically (remember last lecture...)
%	\bit
%		\item Sufficient condition: $\beta R E\left[(Ge^{\nu})^{-\sigma}\right]<1$ (Deaton, Econometrica 1991)
%	\eit
%	
...
\begin{frame}{Value function iteration: updating the guess}
...
\bit
\setlength\itemsep{0.5em}
\item Survey data, e.g., CPS, PSID (US)
\item Register data, e.g., Swedish income registers
\eit

\item Calibration of $\beta, \sigma, \underbar a$ is typically done by method of moments using data on household income and consumption
\bit
\setlength\itemsep{0.5em}
	\item Survey data, e.g., PSID-CEX (US)
...
\begin{eqnarray}
Var(\Delta y_{t}) = Var(\nu_t) + 2Var(\epsilon_{t}) \nonumber \\
Cov(\Delta y_{t}, \Delta y_{t-1}) = -Var(\epsilon_{t}) \nonumber 
\end{eqnarray}

\item Typically, we apply this estimator to income growth residuals in household panel data

\item This method can be generalized in several dimensions, and recent administrative data have taught us a lot:
\bit
	\item Guvenen-Karahan-Ozkan-Song (Econometrica 2021): income growth residuals exhibit fat tails and significant skewness (normality is a bad assumption)
...
%
%	\item Populate an economy with, say, 1000 households, and simulate the economy using the stochastic shock processes and the decision functions obtained by solving the household problem
%	
...
%\bit
%\setlength\itemsep{0.5em}
%\item $\underbar a^{nbl}$ ensures that all borrowing is risk-free 
%\item $\underbar a^{nbl}$ is determined by the condition where the household needs to set $c=0$ for all future periods to pay off its debt
...
\item To solve for $c=c(m)$, use Euler equation and iterate on the budget constraint and the law-of-motion for $m$:
\begin{eqnarray}
&& a_{t+1} = m_t - c_t \nonumber \\
...
\item Substituting and iterating forward gives us
\begin{eqnarray}
...
\item $\Rightarrow$ households behave according to the permanent-income hypothesis (PIH)
\eit
\end{frame}
...
%\eit
%
%\item Now: let's look at what happens in the buffer-stock savings model
%
%\eit
%\end{frame}
...
\bit
	\item Parameter values: $\beta = 0.95$, $\sigma = 1.5$, $R=1.04$, $G=1.03$, $\underbar a = 0$, with $\sigma_{\epsilon}, \sigma_{\nu}$ taken from Gourinchas-Parker (Econometrica, 2002)
...
\bit
	\item Result 1: with income risk, households consume less than they would without income risk
		\bit
			\item with income risk, assets have insurance value
			\item without income risk, households run down current assets quickly due to impatience
		\eit
\eit
...
\bit
\item Result 2: with income risk, the consumption function is concave
\bit
	\item Equivalently: with income risk, the MPC out of current income is larger and decreasing
	\item As the household approaches the constraint, the MPC out of current income grows to $1$
\eit
\eit
...
\bit
\item With a lot of assets, households behave as if there were no income risk, and $MPC \to \left(1-\frac{G}{R}\right)$
\eit
...
\bit
\item Result 3: because of concavity, there is a target buffer stock of assets
	\bit
...
\item Move on to analyze how to use the model to interpret the data

\item Early literature (Zeldes, Deaton, Carroll, and others) focused on testing the buffer-stock model against the PIH model (this description does not fit all papers)
\bit
\setlength\itemsep{0.5em}
	\item Perhaps ex post unsurprisingly, PIH was rejected in several dimensions
	
	\item In the late 1980s/early 1990s, numerical solutions were still difficult $\Rightarrow$ calibrated/estimated models were not used much
\eit
...
\item We will focus on the second-generation literature
...
\bit
\setlength\itemsep{1.5em}
\item Q: How do consumption-savings dynamics evolve over the life cycle?
\bit
	\item Does buffer-stock savings behavior or the permanent-income hypothesis provide a better fit of the data? Does it depend on age?
\eit

\item Method: estimate a structural life-cycle model with idiosyncratic income risk
\bit
\setlength\itemsep{0.5em}
	...
	\item Estimate preference parameters to match profiles
...
P_{t} &=& P_{t-1}G_t e^{\nu_{t}} \nonumber 
\end{eqnarray}
note that $G_t$ has a $t$ subscript
...
\bit
\setlength\itemsep{1em}
\item $R=1.034$ is taken from the average return on safe bond assets
...
%	\item The retirement rule can be estimated directly in the data
	...
	\item Set $\beta$ and $\sigma$ so that the model mimics this profile as closely as possible
\eit

\item GP do this procedure for 16 different educational-occupational groups, but here we only focus on the results for the total population
...
\bit
\setlength\itemsep{0.5em}
	\item The hump shape in consumption directly rejects the hypothesis that working-age households have full insurance - why?
\eit
...
\bit
\setlength\itemsep{0.5em}
\item Matching the consumption-income profile pins down $\beta$ and $\sigma$
\eit
...
\bit
\item By turning income risk on and off in the model, GP decompose household savings behavior into its \emph{life-cycle} and \emph{precautionary} components
\eit
...
\bit
\setlength\itemsep{2em}
\item Young households are buffer-stock savers, while old households are similar to PIH savers 

\item Young households: start with no buffer and retirement is far away $\Rightarrow$ saving primarily reflects precautionary behavior

\item Middle-aged households: already have a buffer and retirement is near $\Rightarrow$ saving primarily reflects consumption smoothing around retirement
...
\bit
\setlength\itemsep{2em}
\item An early paper estimating a consumption-savings model to match key moments in microdata

\item A large literature has followed

\item Two related recent papers:
\bit
\setlength\itemsep{1em}

\item Boar (ReStud 2021): estimates an OLG version of the model using PSID data 
	\bit
		\item Finds that a significant part of household buffers is explained by children's income risk
	\eit

\item Druedahl and Martinello (ReStat 2020): use Swedish register data to estimate consumption responses to exogenous bequest shocks 
\bit
\setlength\itemsep{0.5em}
	\item They find that households quickly consume these bequests $\Rightarrow$ suggests low $\beta$
	
	\item Still, these households typically have sizable wealth holdings
...
\begin{frame}{Blundell-Pistaferri-Preston (AER 2008): The question}
...
\bit
\setlength\itemsep{0.5em}
\item In the 1980s, US income inequality grew a lot
\item Consumption inequality also grew, but at a lower pace, and the gap between consumption and income inequality increased significantly in the late 1980s/early 1990s
...
\bit
\setlength\itemsep{1em}
\item Buffer-stock savings theory teaches us that consumption responds very differently to permanent versus transitory income shocks
...
\item For the third part, BPP suggest estimating the effect using a linear consumption rule
\bit
\setlength\itemsep{0.5em}
	\item People sometimes call this a ``semi-structural'' approach; here, ``semi'' mostly means ``linear''
\eit

\item The paper makes two contributions
\ben
	\item Show how to impute consumption in PSID (panel data on income) using food Engel curves
	\item Use consumption-augmented PSID to estimate the evolution of permanent/transitory income risk and pass-through to consumption
\een
...
\item Second, assume residualized income evolves according to this process (the paper uses a slightly more general process, but it does not affect the point here)
\begin{align*}
...
\end{align*}
\bit
\item Note 1: the treatment effect is time-varying 
...
\item $\Rightarrow$ A rise in income growth inequality without a corresponding decline in autocovariance means that permanent, and not transitory, income variance has increased
...
\item We can then use changes in consumption growth variance:
\begin{align*}
\Delta Var(\Delta c_{it}) &= Var(\nu_{it})\Delta \phi^2_{t} + \phi^2_{t-1} \Delta Var(\nu_{it}) \\
&\quad + Var(\epsilon_{it}) \Delta \psi^2_{t} + \psi^2_{t} \Delta Var(\epsilon_{it}) 
\end{align*}
to estimate $\phi_t, \psi_t$ given the income variances

\eit

\end{frame}

\begin{frame}{BPP: Pass-through coefficients, full sample}

	\begin{figure}
		\centering
		\includegraphics[width=0.9\textwidth]{figures/blundell_tab6_part3.pdf}
	\end{figure}
	
	\end{frame}


\begin{frame}{BPP: Permanent and transitory income variance, full sample}

\begin{figure}
	\centering
	\includegraphics[width=0.9\textwidth]{figures/bpp_shocks_whole_sample.pdf}
\end{figure}

\vspace{-1.4em}
{\scriptsize Note: Own illustration based on tables in BPP. Permanent-shock endpoints are reported for 1979--81 and 1990--92; transitory-shock endpoint is reported for 1990--92.}
...
\bit
\setlength\itemsep{0.3em}
\item In the early 1980s, US consumption inequality tracked income inequality because income inequality growth was driven by permanent-income inequality growth

\item The series diverged from the mid-1980s onward, as income inequality growth stemmed from transitory inequality growth, against which households are much more insured
\eit
...
\bit
	...
	\item An increase in income inequality driven by transitory-income shocks is much less severe than one driven by permanent-income shocks
\eit

\item See Heathcote-Storesletten-Violante (RED 2023) for documentation of similar facts for later decades
...
\bit
\setlength\itemsep{1em}
\item Great virtue of BPP: robust to i.i.d. measurement error in income

\item Big problem of BPP: postulated consumption rule implies random-walk w.r.t. all shocks; not consistent with buffer-stock savings model

\item A theoretically consistent approach must allow for state-dependence, e.g., 
\begin{align*}
	\Delta c_{it}(m_{it}) &= \phi_{t}(m_{it}) \nu_{it} + \psi_{t}(m_{it}) \epsilon_{it} + \xi_{it}
\end{align*}
but in survey data, there is not enough power to estimate state-dependent parameters

\item Nowadays, we have access to large-scale administrative data with minimal measurement error in income

\item Carlsson-D'Amico-Oberg-Skans-Walentin (2026): derive an optimal estimator for state-dependent pass-through cofficients under no measurement error in income, maintaining the assumptions of the canonical buffer-stock savings model
...
\begin{frame}{Carlsson-D'Amico-Oberg-Skans-Walentin (2026)}

	\begin{figure}
		\centering
		\includegraphics[width=0.9\textwidth]{figures/OLS_vs_coworker_gradient_singles_MA2.png}
	\end{figure}
...
\begin{frame}{Kaplan-Violante (Econometrica 2014): Motivation}

\bit
\setlength\itemsep{1em}
\item Natural experiments suggest that the within-quarter \emph{aggregate} MPC out of transitory income shocks is quite large

\item In 2001 and 2007-2008, the US government provided cash transfers to households as part of stimulus programs, with random timing of the transfer

\item Johnson-Parker-Souleles (AER 2006) and Parker-Souleles-Johnson-McClelland (AER 2011) exploit random timing to estimate MPCs of approximately $0.25$

\item As we saw above, this seems corroborated when applying BPP to high-quality admin data
...
\item However, households with little net assets account for a small share of total consumption $\Rightarrow$ cannot explain why aggregate MPC is large

\item How to make sense of this?

\item Aggregate MPC is a key moment for understanding business cycles and the macroeconomic effect of stabilization policies
\eit
...
\item It is primarily access to liquid wealth that should determine households' ability to insure against transitory income shocks
...
\item Define wealthy HtM as HtM households with net illiquid assets $>x$ dollars ($x=0, 1000, 3000$)
...
\begin{frame}{Kaplan-Violante: Full model additions}

\bit
\setlength\itemsep{1em}

\item Their full model also includes
\bit
...
%\bit
%	\item When not making a transaction, the household behaves as in a standard buffer-stock model
...
\item Since $R^{\text{ill}}>R^{\text{liq}}$, saving in illiquid assets maximizes lifetime consumption
...
\item If the return difference is large enough, households put all cash on hand into the illiquid account when investing, becoming liquidity constrained thereafter
...
\bit
\setlength\itemsep{1em}
\item In conventional representative-agent macro models, MPC is low, and fluctuations in income therefore play little role in fluctuations in aggregate demand
\bit
\setlength\itemsep{0.5em}
...
\item An alternative theory to explain high aggregate MPC: some households are simply not very optimizing and consume directly out of what they receive, independently of their financial situation
\bit
\setlength\itemsep{0.5em}
	\item See, e.g., Campbell-Mankiw (NBER Annual, 1989)
	
	\item Fagereng-Holm-Natvik (AEJ Macro 2021): using Norwegian register data on consumption responses to lottery winnings, they show that aggregate MPC declines smoothly over time
	
	\item Auclert-Rognlie-Straub (JPE 2025): an alternative behavioral theory has a hard time explaining the time pattern in MPC, but KV's two-asset model can 
\eit

\item Recent exploratory approaches to understanding heterogeneity in MPCs 
\bit
	\item Aguiar-Bils-Boar (REStud 2025), Colarieti-Mei-Stantcheva (2025), Carlsson-D'Amico-{\"O}berg-Skans-Walentin (2026b)
\eit
\eit
...
\item There is a large literature - we have only glanced at some applications today

\item Accompanied by a large literature on estimating household earnings dynamics
%\bit
...
\item Research in this area operates at the intersection of theory and microdata, often using both structural and reduced-form approaches

%Some additional inspiration: 
%\bit
%\setlength\itemsep{0.5em}	
%	\item Krueger-Perri (ReStud, 2006): BS model with endogenous credit constraint can tightly fit the evolution of income and consumption inequality in the US
%
%	\item Berger-Vavra (Econometrica, 2015): BS model with irreversible durable investments suggests that aggregate MPC with respect to durable goods declines in bad times
%	
...
\item Next lecture: how do incomplete-market households interact with the aggregate economy?
\bit
