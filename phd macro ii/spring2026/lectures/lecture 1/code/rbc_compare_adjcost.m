...
legend('phi = 0','phi = 1','phi = 5','phi = 10')
ylabel('Percent')
%axis([1 T 0 1])
...
subplot(R,C,5)
plot(1:T, 100.*squeeze(qhat(1,1:T)), ':', 'Linewidth', 2); hold on;
plot(1:T, 100.*squeeze(qhat(2,1:T)), '-.', 'Linewidth', 2); hold on;
plot(1:T, 100.*squeeze(qhat(3,1:T)), '--', 'Linewidth', 2); hold on;
plot(1:T, 100.*squeeze(qhat(4,1:T)), '-', 'Linewidth', 2); hold on;
title({'\fontsize{12}Hayashis q'}); grid on
ylabel('Percent')
