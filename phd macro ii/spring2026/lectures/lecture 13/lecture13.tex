...
\title[Incomplete Markets in General Equilibrium]{\large Macroeconomics II, Lecture XIII:\\
...
\item PE enables us to explore how household consumption-saving decisions respond to income, risk, price changes, credit availability, etc.
...
\item Currently, research using business-cycle models with heterogeneous agents is growing rapidly
...
%\ben
%	\item Deterministic transition dynamics with unexpected shocks
%\een
...
\sectiondivider{The Aiyagari Model}
...
\bit
	\setlength\itemsep{2em}
\item The Aiyagari model $=$ neoclassical growth model with a continuum of households that face uninsurable idiosyncratic income risk 
...
\item Households that solve an income-fluctuations problem
\bit
	\item Determines supply of labor and assets, demand for consumption
\eit

\item Competitive firms that maximize production using a production function
\bit
...
\item A unit mass of households, indexed by $i$, solve an income-fluctuations problem
...
\item $U(C)$ satisfies the usual regularity conditions
...
\item This formulation of the income process allows for an arbitrary level of persistence (in contrast to the transitory-permanent income process considered in last lecture)
\bit
	\item See Tauchen (EL 1986) and Tauchen-Hussey (Ecmtra 1991) for how to map a continuous AR process into a discrete-state Markov process
\eit
...
\bit
	\item If we know household $i$ is unemployed in period $t$, then $ \pi_{it}(\epsilon) = \left( \begin{array}{c}
	1 \\
	0
	\end{array}\right)$
\eit
...
\item If $T$ is {\bc regular} (all entries of $T^n$ for some $n>0$ are positive), $\Pi^{*}(\epsilon)$ is given by
\begin{eqnarray}
...
\bit
\setlength\itemsep{0.5em}
\item The savings instrument in this economy is capital, just as in Lectures I-II
...
\item The representative firm rents labor and capital to produce consumption goods $Y$ using a Cobb-Douglas production function:
\begin{eqnarray}
...
\begin{frame}{Rational expectations with heterogeneous households}

\bit
\setlength\itemsep{1.5em}

\item Note: $R_{t+1}$ and $W_{t+1}$ depend on aggregate savings $A_{it+1}$, which, via household decision functions, in turn depends on the joint distribution of $A_{it}, \epsilon_{it}$ 
...
\setlength\itemsep{0.5em}
	...
	\item This appears bothersome; recall the ``curse of dimensionality''
\eit
...
\item An SRCE is a value function $V(A, \epsilon)$, policy functions $C(A, \epsilon), A'(A, \epsilon)$, a distribution $\gamma(A,\epsilon)$, aggregate capital and labor demand $K, L$, and prices $W,R$, s.t.

\ben
\setlength\itemsep{1.5em}

\item Given $W,R$: the value function $V(A, \epsilon)$ and policy functions $C(A, \epsilon), A'(A, \epsilon)$ solve the household Bellman equation

\item Given $W,R$: $K, L$ solve the firm problem

\item The markets for capital and labor clear:
\begin{eqnarray}
K &=& \int_{\bm{\mathcal A} \times \bm{\mathcal E}} A d\gamma \nonumber \\
L &=& \int_{\bm{\mathcal A} \times \bm{\mathcal E}} \epsilon d\gamma \color{red} = \sum_{\epsilon \in \mathcal E} \epsilon \Pi^{*}(\epsilon) \nonumber
\end{eqnarray}
...
\begin{frame}{Does an SRCE exist?}

\bit
\setlength\itemsep{2em}

\item Firm and household problems are well-defined: solutions exist and policy functions are continuous in $R$ and $W$
...
\item Restated: do the asset demand and asset supply curves cross at least once?
...
\item Without income risk, $\epsilon=0$, it is straightforward to show that if
\ben
\setlength\itemsep{0.5em}
\item $\beta (1+r) > 1$, consumption and savings grow indefinitely
\item $\beta (1+r) = 1$, consumption is constant, savings is bounded
\item $\beta (1+r) < 1$, consumption and savings decrease until the borrowing constraint binds; in the limit, $C=\bar Y$
\een
...
\item With uninsurable income risk, households have a stronger savings motive

\item Implication: assets and consumption grow without bound at lower levels of the interest rate $R$
...
\begin{eqnarray}
V_a(A_{max}, \epsilon_N) &\geq& E\left[V_a(A'(A_{max}, \epsilon_N), \epsilon')\right]\nonumber\\
&>& E\left[V_a(A_{max}, \epsilon_N)\right]\nonumber \text{ using that } V_{aa}<0 \nonumber\\
&=& V_a(A'_{max}, \epsilon_N) \nonumber 
\end{eqnarray}
which is a contradiction. Thus $A'$ does not have an upper bound $\Rightarrow$ asset holdings diverge to infinity
...
\item Asset demand is still given by the same firm F.O.C.: $\alpha \left(\frac{K(r)}{L}\right)^{\alpha-1} = r+\delta $
...
%\begin{frame}{Is the SRCE unique?}
%
%\bit
%\setlength\itemsep{2em}
%
%\item Under the assumptions made, there are no results on whether $E(a(r))$ is monotonically increasing
%
...
\item Set an initial $r_0$ that satisfies $-\delta<r_0<\frac{1}{\beta}-1$
...
\end{eqnarray}
\bit
	\item Note: this involves picking an initial distribution $\gamma_0$ and then simulating using the computed policy functions
\eit
...
%
%\item We have not proved this here, but practical experience suggests that this is usually the case
%
%\eit
%
%\end{frame}
...
\item In the case of zero net asset supply (e.g., no government debt, money, foreign borrowing, etc.), asset market clearing is simply
\begin{eqnarray}
...
\item The Aiyagari diagram is the same, except that the aggregate demand curve is vertical at zero
\bit
...
\item Now that we have an incomplete-markets stationary equilibrium, let's put it to work
...
\item Taking a realistic income process as given, can the model explain observed wealth inequality?

\item This is a theory of wealth inequality entirely based on luck

\item Some agents will experience lucky income draws and accumulate assets to insure against bad draws in the future

\item Some agents will experience unlucky income draws and deaccumulate assets or borrow to smooth consumption
...
\begin{figure}
	...
	\caption*{\footnotesize Numbers measured in thousands of dollars. From D\'iaz-Gim\'enez-Glover-R\'ios-Rull (FED Minneapolis 2011). Data from SCF.}
\end{figure}

\begin{figure}
	\centering
	\includegraphics[scale=0.4]{figures/dgr_tab2.pdf}
	\caption*{\footnotesize From D\'iaz-Gim\'enez-Glover-R\'ios-Rull (FED Minneapolis 2011). Data from SCF.}
\end{figure}

\bit
	\item Main takeaway: the wealth distribution is much more concentrated than the income distribution
\eit
...
\setlength\itemsep{2em}
	...
	\item Find a process $\mathcal E = \{\epsilon_0, ..., \epsilon_N\}$, $\pi(\epsilon, \epsilon')$ that matches the autocorrelation and variance of $\nu_{it}$ 
	\een
...
\bit
	\item It would be very surprising if it could explain everything.
\eit

\item The failure is due to problems at both ends of the wealth distribution:
\ben
\setlength\itemsep{0.5em}
\item Very few households are close to the borrowing constraint
\bit
	\item The welfare loss of not being able to smooth at all is large
\eit
\item Very few households have savings several multiples of income at the top of the distribution
\bit
	\item This is only rational if you face a substantial risk of losing all your income for a long time
\eit
\een
...
\bit
\setlength\itemsep{1em}
\item Adding means-tested social insurance eliminates the saving motive among the income-poor (Skinner-Hubbard-Zeldes, JPE 1995)
...
\bit
	...
	\item In the data, a large fraction of top wealth is held by entrepreneurs. Quadrini (RED 2000) and Cagetti-De Nardi (JPE 2006) model entrepreneurs as households with access to high-return investments
	
	\item See also Krusell-Hubmer-Smith (NBER Macro Annual 2020); Bylund (2026)
	
\eit

\item Non-zero probability that very rich people lose it all (D\'iaz-Gim\'enez-Glover-R\'ios-Rull, JPE 2003)
\bit
...
\item An unconstrained social planner would insure all households against idiosyncratic income risk $\Rightarrow$ constant consumption for all households in the stationary state $\Rightarrow$ $\beta (1+r) =1$
\bit
	\item An unconstrained social planner effectively reintroduces the missing market for a complete set of Arrow securities  
\eit
...
\bit
	...
	\item D\'avila-Hong-Krusell-R\'ios-Rull (Ecmtra 2012): The Aiyagari model is not constrained efficient; the planner wants to instruct well-off households to save more, raising the wage of poor households
...
\item Flod\'en-Linde (RED 2001) studies redistributive taxation
\bit
	\setlength\itemsep{0.5em}
	...
	\item Flod\'en and Linde add a labor supply decision to the Aiyagari model and find an optimal level of labor taxes $\tau \approx 0.27$ for a US calibration  
\eit

\item A progressive income tax schedule is an even more effective insurance device
\bit
	\setlength\itemsep{0.5em}
	\item Optimal progressivity is studied by B\'enabou (Ecmtra 2002); Conesa-Krueger (RED 2006); Krueger-Ludwig (AER 2013); Heathcote-Storesletten-Violante (QJE 2017)
\eit
...
\bit
	...
	\item Aiyagari and McGrattan find that these effects largely offset each other, and the welfare effects from varying the overall debt level are small
\eit
...
\item Generally, analyzing dynamics requires computational techniques that we do not practice in this course (covered in the first second-year course)


\item Central problem: the entire wealth distribution is an infinite-dimensional state variable

\item Useful to know: two computational approaches:
\ben
\setlength\itemsep{1em}

\item Approximate equilibrium (Krusell-Smith JPE, 1998): exploit that knowing just a few moments of the wealth distribution is sufficient to make very good forecasts of prices

\item Linearization (Boppart-Krusell-Mitman, JEDC 2018; Auclert-Bardoczy-Rognlie-Straub, Ecmtra 2021): transition dynamics (IRFs) to a one-time unexpected shock are a sufficient statistic for solving a first-order approximation, just like in representative-agent RBC
\bit
	\item Computing transition dynamics to a one-time unexpected shock is a bigger system of equations, but conceptually the same as computing the steady state 
...
%
%\begin{frame}{Transition dynamics to an MIT shock}
%\bit
%\setlength\itemsep{1.5em}
%
%\item Consider our Aiyagari model which rests in steady state at period $-1$, but where TFP $Z_t$ is unexpectedly hit by a shock $\xi$ in period 0 and then evolves autoregressively:
%\begin{eqnarray*}
...
%\end{frame}
%
%%\begin{frame}{Transition dynamics to MIT shock}
%%\bit
%%\setlength\itemsep{1.5em}
%%
%%\item Consider our Aiyagari model, but where we add a tax-and-transfer system
%%\bit
...
%
%\item Calculate $\max_{0<t<T} |\hat A_{agg, t+1}-\hat K_{t+1}|$. If sufficiently small, stop. If not sufficiently small, repeat with new guess $\hat K_{t+1, new} = \frac{1}{2} (\hat K_{t+1} + \hat A_{agg, t+1})$
%
%\een
%
%\end{frame}
...
%\bit
%\setlength\itemsep{1.5em}
%\item Having computed deterministic transition dynamics, computing the first-order approximation of an economy with an aggregate shock process is straightforward (Boppart-Krusell-Mitman, JEDC 2018)
%
%
...
%\bit
%\setlength\itemsep{1.5em}
%\item Although your model might have a lot of nonlinearities at the micro level, aggregate variables might still be close to linear in response to aggregate shocks, making first-order approximation a valid approach
%
%\item As indicated, linearization methods are a quite recent phenomenon (quite surprisingly)
%
...
\item Aiyagari model = neoclassical growth model with uninsurable earnings risk
...
\bit
\setlength\itemsep{0.5em}
	\item The literature using incomplete-market models to investigate macroeconomic dynamics has exploded

	\item To conclude the course, I will briefly talk next class about one area of rapidly evolving research: Heterogeneous-Agent New-Keynesian (HANK) models
	\bit
