...
\newcommand{\includefigureorplaceholder}[2][]{%
  \IfFileExists{#2}{\includegraphics[#1]{#2}}{%
    \fbox{\footnotesize Missing figure: \texttt{\detokenize{#2}}}%
  }%
}
...
\item Evidence concerning the effects of monetary policy
...
\begin{center}
	\huge Evidence concerning the effects of monetary policy \normalfont
\end{center}
...
\setlength\itemsep{0.5em} 
	\item Historical analysis of unusual episodes, e.g.,
		\bit
			...
			\item Sweden's experiment with using monetary policy for counteracting household debt (Coglianese-Olsson-Patterson, AER 2026)
		\eit
...
\item $\mathbf{A}_0$ captures the contemporaneous causal relationships between the endogenous variables 
...
\item $\Rightarrow$ the contemporaneous causal effect of MP on output/inflation is zero; justifies placing restrictions on $A_0$
...
\item High-frequency identification (Kuttner, JME 2001; Nakamura-Steinsson QJE 2018)
\bit
\setlength\itemsep{0.5em} 
\item Idea: discontinuous jumps in price of interest-rate forward contracts around policy announcement reflect an unexpected policy change
...
\begin{figure}
	\centering
	\includefigureorplaceholder[scale=0.9]{Figures/Ramey_fig1a.pdf}
...
\begin{figure}
	\centering
	\includefigureorplaceholder[scale=0.9]{Figures/Ramey_fig2c.pdf}
	\caption*{US monthly data, from Ramey (HB Macro 2016)}
\end{figure}
...
\begin{figure}
	\centering
	\includefigureorplaceholder[scale=0.9]{Figures/Ramey_fig3a.pdf}
	\caption*{US monthly data, from Ramey (HB Macro 2016)}
\end{figure}
...
\begin{eqnarray}
\max_{\{C_t, N_t, B_{t+1}\}} && E_0 \sum_{t=0}^{\infty} \beta^t \left[U(C_t)- V(N_t) \right]\nonumber \\
...
\begin{frame}{Household problem optimality conditions}
...
\begin{frame}{Log-linearizing household optimality conditions}
...
\item Note: $\log$ utility implies that the elasticity of current consumption w.r.t. the real interest rate is $-1$
...
\item Carlsson-Nordstr{\"o}m Skans (AER 2013) exploit Swedish administrative data
\bit
...
\item If inflation happens to be ``too large'' $\Rightarrow$ central bank raises interest rate $\Rightarrow$ inflation tomorrow rises

\item Put differently, the central bank ``threatens'' explosive dynamics
\eit
...
\bit
\setlength\itemsep{1em}
\item Back in the day: Quantity theory
\bit
\setlength\itemsep{0.5em}
\item See, e.g., Milton Friedman's AEA presidential address (AER 1968)
...
\item Problem: $v$ is highly endogenous to the interest rate (at $i=0$, bonds are perfect substitutes to money) and $M$ is not controllable anymore
\eit
...
\begin{figure}
	\centering
\includefigureorplaceholder[scale=0.5]{barro_bianchi_2024_figure.pdf}
\end{figure}
...
\item $\theta=2/3$ to match average price duration of three quarters (Gal\'{i}-Lop\'{e}z-Salido EER 2001)
...
\begin{figure}
\centering
\includefigureorplaceholder[scale=0.5,trim= 0 200 0 200, clip]{figures/nk_monshock_4variables.pdf}
\end{figure}
...
\begin{figure}
\centering
\includefigureorplaceholder[scale=0.5,trim= 0 200 0 200, clip]{figures/nk_monshock_9variables.pdf}
\end{figure}
...
%\ben
%\setlength\itemsep{0.5em}
%\item Monetary factors do not affect the natural real interest
%
%\item Inefficient fluctuations are caused by deviations in the real interest rate from the natural real interest rate
%\een
