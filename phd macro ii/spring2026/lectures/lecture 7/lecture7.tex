...
\item We learned a lot, but the models we considered had several problems and are obviously too stylized to speak to many macroeconomic phenomena

\item In particular, two assumptions seem very unrealistic while also being important for understanding macro dynamics
\ben
\setlength\itemsep{0.5em}
	\item Labor markets are frictionless
	
	\item Households have access to complete asset markets (and can therefore be summarized by a representative agent)
\een 

\item Remainder of the course introduces a set of models that relax these assumptions

\item Besides developing a deeper theory of business cycle dynamics, these models will allow us to explore an additional set of questions in macro research:
\bit
...
\item To get us started with frictional labor markets, we'll first look at a few key facts
...
\sectiondivider{Labor market facts}
...
\item Main focus is the US, but we will also take a quick glance at other OECD countries
...
\item Fact 2: considerable heterogeneity in fluctuations across countries
\eit
...
\item Using data on unemployment levels by duration, we estimate the probability $F_t$ that an unemployed worker finds a job within a month:
\begin{eqnarray}
1-F_t = \frac{\text{\#Unemployed with duration>1 in month t+1}}{\text{\#Unemployed in month t}} \nonumber
\end{eqnarray}

\item Similarly, the monthly job-separation probability can be estimated from
\begin{eqnarray}
...
\begin{figure}
	...
	\caption*{\footnotesize From Elsby-Hobijn-Sahin (ReStat 2013).}
\end{figure}
...
\item How large are fluctuations in wages?
...
\begin{frame}{US wage dispersion in CPS data}
...
\bit
\item Fact: rising wage inequality 1975-2000
\eit


\end{frame}

\begin{frame}{US wage dispersion: update through 2024 (CPS ASEC)}

\begin{figure}
	\centering
	\includegraphics[scale=0.44]{figures/acemoglu_2002_fig2_update_2024.pdf}
	\caption*{\footnotesize Own CPS ASEC calculations for white male full-time full-year workers aged 18-65, earnings years 2000-2024.}
\end{figure}

\bit
\item Earlier CPS ASEC-based update (shorter post-2000 window)
\item Useful benchmark alongside ORG/MORG results
\eit

\end{frame}

\begin{frame}{US wage dispersion: update through 2024 (ORG/MORG)}

\begin{figure}
	\centering
	\includegraphics[scale=0.44]{figures/acemoglu_2002_fig2_morg_2024.pdf}
	\caption*{\footnotesize Own CPS ORG/MORG calculations for white male full-time wage and salary workers aged 18-65, 1979-2024; hourly wage = weekly earnings/usual hours.}
\end{figure}

\bit
\item Fact: upper-tail inequality keeps rising through 2024
\item The lower tail recovers late in the sample, but much less than the 90th percentile
\eit
...
\bit
\setlength\itemsep{1.5em}
\item How much of the observed wage dispersion is due to differences in observable characteristics?

\item Cross-sectional household survey data (e.g. CPS, PSID)

\item Mincer regression
\begin{eqnarray}
\log y_{it} = \mu_y + \beta \mathbf{X}_{it} + \epsilon_{it} \nonumber
\end{eqnarray}
where $y_{it}$ is earnings and $\mathbf{X}_{it}$ includes age, occupation, industry, tenure, education, etc.
...
\begin{frame}{US residual wage dispersion in CPS data)}
...
\bit
	\item Fact: Rising residual wage inequality 1975-2000
...
\begin{frame}{US residual wage dispersion: update through 2024 (CPS ASEC)}

\begin{figure}
	\centering
	\includegraphics[scale=0.43]{figures/acemoglu_2002_fig3_update_2024.pdf}
	\caption*{\footnotesize Y-axis: log points. Own CPS ASEC calculations for white male full-time full-year workers aged 18-65, earnings years 2000-2024.}
\end{figure}

\bit
\item Earlier CPS ASEC-based residual series for comparison
\item ORG/MORG provides the longer historical perspective
\eit

\end{frame}

\begin{frame}{US residual wage dispersion: update through 2024 (ORG/MORG)}

\begin{figure}
	\centering
	\includegraphics[scale=0.43]{figures/acemoglu_2002_fig3_morg_2024.pdf}
	\caption*{\footnotesize Y-axis: log points. Own CPS ORG/MORG calculations, white male full-time wage and salary workers, 1979-2024.}
\end{figure}

\bit
\item Fact: residual inequality remains high well after 2000
\item Recent widening mainly comes from the upper half of the residual wage distribution
\eit

\end{frame}


\sectiondivider{Search models: overview}
...
\item Search markets: markets in which buyers (employers) and sellers (workers) spend time/resources searching for each other, and where there is some element of randomness in when and/or with whom a match happens

\item Because searching takes time, some workers will experience periods without work: {\color{blue}frictional unemployment}

\item Because matching has an element of randomness, some workers will be luckier than others in finding good jobs: {\color{blue}frictional wage dispersion}
...
\item The search approach to labor markets was developed in the 60's, first largely focused on partial-equilibrium search behavior and later built into general-equilibrium frameworks
...
\sectiondivider{Mathematical Preliminaries}
...
\item For a non-negative random variable $x$ with cdf $F(x)$ on support $[x_1, x_2]$, we can define the expected value as 
\begin{eqnarray}
Ex = \int_{x_1}^{x_2} x d F(x) \nonumber
\end{eqnarray}

\item Contrast with the ``standard'' definition
\begin{eqnarray}
...
\item However, when $F$ has mass points, the expected value is not well defined under the latter definition {\rc (Draw example on whiteboard)}
...
\item Consider our definition of expected value
\begin{eqnarray}
Ex = \int_{x_1}^{x_2} x d F(x) \nonumber
\end{eqnarray}
...
\item Properties of RS integrals:
\ben
...
\sectiondivider{The McCall model}
...
\item An economy populated by a measure $P$ of ex ante identical, infinitely lived agents with a linear utility function and discount factor $\beta$

\item Households can either be {\bc employed} or {\rc unemployed}

\item An {\rc unemployed} household receives benefits {\rc $b$}, while an {\bc employed} household receives wage income {\bc$w$}

\item No savings technology, agents consume hand-to-mouth

\item Unemployed households receive a job offer with probability $\Lambda_{u}$ in each period, with wages $w$ drawn from i.i.d. $F(w)$ with finite support $\mathds{W}=[ w_{min}, w_{max}]$   

\item Employed households separate from jobs at exogenous probability $S$

\item Household problem: if unemployed and receiving a job offer, accept it or wait
\eit
...
\item Search models are generally easier to work with in {\bc recursive formulation}
...
\item flow value $rW$ $=$ flow benefit $+$ flow probability of a shock $\times$ change in value
...
\item Now we 1) solve this problem and 2) study the implications of the solution
...
\item $\frac{w-r U}{r+\sigma}$ increasing, $0$ is a constant {\rc(Draw graph on whiteboard)}
...
\item Because of the reservation strategy, the unemployed value can be rewritten
\begin{eqnarray}
...
\end{eqnarray}
\item We call this the {\bc reservation wage equation}
\bit
\setlength\itemsep{0.5em}
...
\item Reminder: the problem was to find the acceptance rule, which we have shown to be a cutoff strategy with reservation wage $w_R$
...
\item Apply Leibniz's rule:
\begin{eqnarray}
T'(w_R) &=& \frac{\lambda_u }{r+\sigma}\left[(w_{max}-w_R)\frac{\partial F(w_{max})}{\partial w_R}\right. \nonumber \\
&& \left.- (w_{R}-w_R)\frac{\partial F(w_{R})}{\partial w_R} + \int_{w \geq w_R} -1 dF(w)\right] \nonumber \\
&=& -\frac{\lambda_u }{r+\sigma}\int_{w \geq w_R} dF(w) \nonumber \\
&<& 0 \nonumber
\end{eqnarray}

\item Ergo, {\bc the reservation wage increases with unemployment benefits}
\eit
...
\item The economy is populated by $P$ workers
...
\item Given $\sigma, \lambda$, this {\bc ordinary differential equation} has a unique solution converging to the steady-state value $u$.
...
\item Following these insights, a large empirical literature has used micro data to investigate the effect of unemployment benefits on unemployment incidence, unemployment duration, and match quality

\item Reservation wages and job-search behavior
\bit
	\setlength\itemsep{0.5em}
	\item Krueger-Mueller (AEJep 2016) use interview data to document self-perceived reservation wages
	\item Marinescu-Skandalis (QJE 2020) use online application data to document how job-search behavior changes with benefit duration
\eit

\item Unemployment duration and benefit generosity:
\bit
	\setlength\itemsep{0.5em}
	\item Older literature summarized in Krueger and Meyer (Handbook PE 2002)
	
	\item Card-Lee-Pei-Weber (Econometrica, 2015) exploit kinks in Austrian UI
...
\bit
	...
	\item Nekoei-Weber (AER 2017) use Austrian micro data and an RD design: positive effect
\eit

\item Lesson that appears in many of these papers: duration dependence matters
\bit
\setlength\itemsep{0.5em}
\item Reservation wage falls as unemployment benefit period is ending; human capital depreciates while unemployed
\eit

\item Ljungqvist-Sargent (JPE 1998) argue that duration dependence is key for understanding differences in unemployment dynamics in Europe vs. the US
%\bit
%\setlength\itemsep{0.5em}
...
\setlength\itemsep{0.5em}
	...
	\item Evidence based on mass layoffs might confound partial- and general-equilibrium effects (Cederl\"of 2019); see Lecture III on the DMP model
\eit

\item In the 70s and 80s, European countries had generous replacement rates tied to previous wages, with close-to-unlimited duration
...
\item In the late 70s and 80s, macroeconomic volatility increased (oil price shocks) $\Rightarrow$ differences in UI institutions can explain why Europe, but not the US, got stuck in a high-unemployment equilibrium with substantial long-term unemployment
\eit
...
\item Hypothesis: some of the empirical wage dispersion that cannot be accounted for by observed heterogeneity is due to this kind of randomness

\item Remember: Mincer regressions typically produce $R^2 < 0.3$ (Mortensen, Book 2003) and substantial residual dispersion: Acemoglu (JLE 2002) reports a 90/10 ratio $\approx 3.3$ and a 50/10 ratio $\approx 1.8$
...
\begin{frame}{Frictional wage dispersion: how to test the model?}

\bit
\setlength\itemsep{2em}

\item To answer this question, the obvious approach seems to be
\ben
\setlength\itemsep{0.5em}
...
\item Mm is a measure of wage dispersion and \bf the formula does not contain $F$! \normalfont

\item Can the model match the data?
\bit
\setlength\itemsep{0.5em}
\item All parameters in our formula are observed in the data!
\item Plug in parameter values and compare the model-implied Mm ratio with the data
\eit
...
\item \bf The model comes nowhere near the data! \normalfont
...
\item The only ingredient is optimal household search behavior $\rightarrow$ a very robust result

\item However, we shall see that one interesting avenue for thinking about frictional wage dispersion is models in which households also perform on-the-job search
...
\item Motivated by these predictions, a large empirical literature has emerged

\item But the natural question is: where did the offer distribution $F$ and contact rate $\lambda_u$ come from?
\bit
...
\begin{frame}{General equilibrium? The Rothschild (JPE 1973) critique}
...
\begin{frame}{Bonus material: mapping data probabilities to model flow rates}
...
\item This does not change anything in our continuous-time model, as the probability of finding $>1$ jobs in any given instant is negligible
\eit
