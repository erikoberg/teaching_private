...
\title[Incomplete markets models: basics]{\large Macroeconomics II, Lecture XI:\\
...
\item Traditional macroeconomic models assume a representative agent: \bf no room for studying the interaction between macroeconomic dynamics and inequality \normalfont

\item Inequality and economic heterogeneity are, however, very salient features of the macroeconomic environments we seek to understand
...
\item the distributional implications of macroeconomic events
\bit
\item Does technical change increase income inequality?
\item Does wealth inequality widen when credit becomes cheaper?
\eit

\item how household heterogeneity matters for macroeconomic transmission
\bit
\item Does wider wealth inequality imply more business cycle volatility?
\item Does the existence of more high-debt households imply that monetary policy is more potent?
\eit
\een
...
\begin{figure}
	\centering
\includegraphics[width=\textwidth,height=0.78\textheight,keepaspectratio, trim = 0cm 3cm 0cm 0cm, clip]{figures/piketty_inc_ineq.pdf}
...
\begin{figure}
	\centering
\includegraphics[width=\textwidth,height=0.78\textheight,keepaspectratio]{figures/guvenen_2016_lifecycle.pdf}
...
\begin{figure}
	\centering
\includegraphics[width=\textwidth,height=0.76\textheight,keepaspectratio]{figures/guvenen_2014_recession.pdf}
...
\begin{figure}
	\centering
\includegraphics[width=\textwidth,height=0.78\textheight,keepaspectratio, trim = 0cm 0cm 0cm 0cm, clip]{figures/kuhn2020_fig4.pdf}
...
\item Lecture 12: Buffer-stock savings: model meets data
...
\item {\bc Market structure} refers to which assets are available to the household; it determines how the household can allocate resources across states.
...
\item Complete markets:
\begin{eqnarray}
C_{it}(s^t) + \sum_{s_{t+1}|s^t} Q_t(s^t, s_{t+1}) A_{it+1}(s^t, s_{t+1})
&\leq& Y_{it}(s^t) + A_{it}(s^t, s_t) \quad \forall t, s^{t}  \nonumber 
\end{eqnarray}
...
\item All households live for $T$ periods; $T = \infty$ is possible
...
\item For each event history $s^t$, household $i$ receives endowment $Y^i_t(s^t)$
\eit
...
\item In period 3, there are 8 possible ``event histories'' or ``states of the world'':
\eit

\begin{figure}
\begin{center}
	\small
	\begin{tikzpicture}[scale=0.85, transform shape, thin,
...
\item Note 1: Here, individual endowment depends on the entire event history, $Y^i_t = Y^i_t(s^t)$ 
\item Note 2: Even if income only depends on the realized shock today, $Y^i_t = Y^i_t(s_t)$, consumption and savings choices in period $t$ depend on the entire event history $s^t$
\eit
...
\item A consumption allocation is a collection $\{C_{it}(s^t)\}_{i=1}^N$
...
\item A feasible consumption allocation $\{C_{it}(s^t)\}_{i=1}^N$ is Pareto efficient if there is no other feasible consumption allocation $\{\hat C_{it}(s^t)\}_{i=1}^N$ such that
\begin{eqnarray}
...
\ben
	\item pays off one consumption good at node $\{s^t,0\}$ and costs $Q_t(s^t,0)$
	\item pays off one consumption good at node $\{s^t,1\}$ and costs $Q_t(s^t,1)$ 
\een
\eit
...
\text{s.t.} && 	C_{it}(s^t) + \sum_{s_{t+1}|s^t} Q_t(s^t, s_{t+1}) A_{it+1}(s^t, s_{t+1}) \leq \nonumber \\
&& Y_{it}(s^t) + A_{it}(s^t, s_t) \quad \forall t, s^{t}\nonumber \\
&& A_{it+1}(s^t, s_{t+1}) \geq - \bar A_i \hspace{3mm} \forall t, s^{t}\nonumber
\end{eqnarray}

\item The {\bc First welfare theorem} applies: the allocation is Pareto efficient and therefore coincides with that of a social planner for some Pareto weights $\{\alpha_i\}_{1}^{N}$
...
Q_t(s^t, s_{t+1}) &=& \beta \pi(s_{t+1} | s^t) \frac{U_c(C_{t+1}(s^{t+1}))}{U_c(C_{t}(s^t))} \nonumber
\end{eqnarray}
%\bit
...
\item ... and aggregate dynamics are \bf \emph{as if} \normalfont there were a representative agent who solves
\begin{eqnarray}
\max_{C_{t}(s^t), A_{t+1}(s^t)} && \sum_{t=0}^{T} \sum_{s^t \in S^t} \beta^t \pi (s^t) U(C_{t}(s^t)) \nonumber \\
\text{s.t.} && 	C_{t}(s^t) + \sum_{s_{t+1}|s^t} Q_t(s^t, s_{t+1}) A_{t+1}(s^t, s_{t+1}) \nonumber \\
&& \leq Y_{t}(s^t) + A_{t}(s^t, s_{t-1}) \quad \forall t, s^{t}\nonumber \\
...
\bit
\setlength\itemsep{0.5em}
	\item A large literature tests the extent of consumption insurance; see Violante's textbook chapter
\eit 
...
\bit
\item At each node $s^t$, one asset that pays off one consumption good in period $t+1$ and costs $Q(s^t)$ can be traded
\eit
...
&& A_{it+1}(s^t) \geq - \bar A_i \hspace{3mm} \forall t, s^{t}\nonumber
\end{eqnarray}
...
\item The remainder of this course is devoted to explaining these two claims and deriving their implications
...
\item Recursive household problem: find value function $V$ and policy functions $C,A'$ that solve
\begin{eqnarray}
...
\item Actually, for i.i.d. shocks, there is only one
...
\item When studying a recursive problem, always ask yourself what the minimal set of state variables is!
\bit
\setlength\itemsep{0.5em}
	...
	\item Note: this is only true when $Y_t$ is i.i.d.
	\bit
...
\begin{frame}{Recursive formulation III}
...
\begin{frame}{Recursive formulation IV}
...
\begin{frame}{Recursive formulation V}
\ben
\setlength\itemsep{1em}

\item Solution is characterized by an Euler equation,
\begin{eqnarray}
...
\ben
\item bind $\Rightarrow$ corner solution with $\mu>0$
\item not bind $\Rightarrow$ interior solution with $\mu=0$
\een
\eit

\item and an envelope condition {\rc (Do on whiteboard)}
\begin{eqnarray}
V_m(M') &=& U_c(C'). \nonumber
\end{eqnarray} 
...
\begin{frame}{Recursive formulation VI}
...
\item a corner solution satisfies
\begin{eqnarray}
C(M)=M \nonumber
\end{eqnarray}
...
\item Therefore, the global solution $C=C(M)$ satisfies
\begin{eqnarray}
...
\item So far: how to characterize consumption-savings problems with uninsurable income risk

\item Now: what does uninsurable income risk imply for consumption-savings dynamics?

\item One lesson we've already learned: households may face a binding credit constraint, in which case they have a high {\bc marginal propensity to consume (MPC)}
...
\item Without loss of generality, assume $\beta R =1$

\item Assume the credit constraint is never binding; the problem is characterized by
\begin{eqnarray}
U_c(C) = E U_c(C') \nonumber
\end{eqnarray}
...
\item Lesson: for the same income stream, prudent preferences imply that consumption is expected to grow over time $\Rightarrow$ the household saves more today!
\eit
...
\item Precautionary savings may also arise due to a {\bc potentially binding credit constraint}
...
\item As before, assume $\beta R =1$ without loss of generality
...
\item If the credit constraint is binding, the household consumes less today than it would like given its consumption-smoothing motive
\eit
...
\item Suppose the credit constraint is not binding today, but that it might be binding tomorrow:
\begin{eqnarray}
C_{t} &=& E_t C_{t+1} \nonumber \\
...
\begin{frame}{Credit constraints IV}

	\bit
	\setlength\itemsep{1em}
	
	\item Lesson: anticipated future binding constraints reduce consumption today relative to the unconstrained consumption-smoothing benchmark

		\item I.e., anticipating a binding credit constraint induces the household to save more
...
\item Which values of $\bar A$ are allowed?
...
\item The precautionary-savings motive is intimately linked to the {\bc marginal propensity to consume (MPC)}
\bit
...
\item Another standard reason why income risk may affect consumption dynamics: irreversible durable purchases
\bit
...
%\setlength\itemsep{1.5em}
%
%\item We have shown how uninsurable income risk creates an additional savings motive (on top of regular consumption smoothing)
%
...
%\ben
%\setlength\itemsep{0.5em}
%\item $\beta R > 1$, consumption and savings grow indefinitely
%\item $\beta R = 1$, consumption is constant, savings is bounded
%\item $\beta R < 1$, consumption and savings decrease until borrowing constraint binds, in the limit $C=\bar Y$
%\een
%
...
\item Complete markets: ex-ante homogeneous households will be ex-post homogeneous

\item Incomplete markets: ex-ante homogeneous households will be ex-post heterogeneous

\item Much of the incomplete-markets literature is organized around the income-fluctuations problem

\item Features of the income-fluctuations problem:
\bit
	\item Stochastic income
	\item Limited set of assets (typically: one risk-free asset)
	\item Potentially binding credit constraint
\eit

\item With incomplete markets, households have a precautionary-savings motive
\bit
	\item Prudence
