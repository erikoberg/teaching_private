...
% Make PDF metadata/bookmarks handle accents (e.g., \"O)
...
\item Two extensions to address the labor wedge
\bit
...
\setlength\itemsep{0.5em} 
	...
	\item Karabarbounis (RED 2014) shows you can sidestep this issue if assuming a Cobb-Douglas production function, finds that LM wedge is mostly due to $W_t \neq MRS$
\eit
...
\begin{frame}{Are Solow residuals a reasonable measure of technology shocks?}

\bit
\setlength\itemsep{1.5em} 

\item We apparently need large fluctuations in the efficiency wedge to explain the data
...
\item Reasonable that technological progress fluctuates around trend, and that Solow residuals capture some of this

\item However, not reasonable that these \emph{detrended} fluctuations can be negative with $-5 \%$ or $-10 \%$ $\Rightarrow$ implies huge technological regress!
\bit
	\item Solow residuals imply technological regress 40\% of the time in post-war US data (King-Rebelo, HBmacro 1999)
\eit
...
\begin{frame}{Cleaning ``Solow residuals'' (I): Utilization}

\bit
\setlength\itemsep{1em} 

\item Firms adjust the usage of their predetermined factors of production over the business cycle
...
\item Both find a technology series with 20-25 \% less volatility than original Solow residuals
\eit
...
\begin{frame}{Cleaning ``Solow residuals'' (II): Misallocation}

\bit
\setlength\itemsep{1em} 


\item Suppose some of the factors of production $N_t, K_t$ are allocated to ``under-performing'' firms
...
\item Let's start by addressing the labor wedge and the efficiency wedge
\bit
\setlength\itemsep{0.5em}
	\item Problem set 5: endogenous capacity utilization to address the efficiency wedge
	
\item Remainder of this lecture: addressing the labor wedge
\eit
...
\begin{center}
	\huge Two extensions to address the labor wedge \normalfont
\end{center}
...
\begin{frame}{Is GHH a reasonable preference specification?}

\bit
\setlength\itemsep{1.5em}
	\item Cesarini-Lindqvist-Notowidigdo-{\"O}stling (AER 2017): labor supply responses to windfall lottery gains very small
	...
	\setlength\itemsep{0.5em}
		...
		\item Broer-Harmenberg-Krusell-{\"O}berg (AER:insights 2023): this is what you should expect if households are subject to rigid wage contracts
	\eit
\eit
...
\begin{frame}{Rigid-wage contracts a la Broer-Harmenberg-Krusell-{\"O}berg}

\bit
\setlength\itemsep{1em}
	\item ``Rigid'' wages = wage contracts cannot be contingent on the shock, nor can they be renegotiated once a shock happens

	\item Instead of having a competitive market for hours in exchange for fixed per-hour wages, suppose we have a competitive market for rigid wage-hour contracts: $W(N)$
	...
	\item $\Rightarrow$ as if GHH within the contract (i.e., demand-determined hours), as if KPR when rewriting the contract (i.e., demand-and-supply-determined hours) 
	\bit
		\item Frequency of recontracting determines how short the short run is
	\eit
\eit
...
\begin{frame}{Equilibrium locus by frequency}
	...
	\begin{figure}
		...
		\begin{minipage}{0.8\textwidth}
			\centering
	%        \caption*{Monthly}
			\includegraphics[width=\linewidth]{Figures/combined_frequency_mqhy_2__fe24.pdf}
		\end{minipage}
	\end{figure}
	...
	\bit
	\setlength\itemsep{0.5em}
		\item Built with monthly payslip data covering the universe of Norwegian workers 2021-2023
		\item From ongoing work with Karl Harmenberg and Maria Olsson
	\eit
...
\begin{frame}{Equilibrium locus by contract type (monthly frequency)}
	...
	\begin{figure}
		...
		\begin{minipage}{0.8\textwidth}
			\centering
	%        \caption*{Monthly}
			\includegraphics[width=\linewidth]{Figures/m_ContractRigidity21_fe22.pdf}
		\end{minipage}
	\end{figure}
...
\item Business cycle accounting (BCA): useful diagnosis tool $\Rightarrow$ we learned that vanilla RBC is associated with large labor and efficiency wedges
\bit
\setlength\itemsep{0.5em} 
	\item It is not reasonable to interpret the entire efficiency wedge as exogenous technological shocks

\item After cleaning Solow residuals from utilization, amplification problem becomes bigger
\eit

\item Motivated by BCA, we learned that model fit is improved if making labor supply more elastic and muting income effects
\bit
	\item Highly elastic labor supply a natural implication of indivisible labor
