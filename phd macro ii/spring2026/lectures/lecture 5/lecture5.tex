...
\title[NK: Shocks \& Propagation]{Macroeconomics II Part II, Lecture V:\\
	  The New-Keynesian Model: Shocks \& Propagation}
\author{Erik {\"O}berg}
\date{}
...
\item For the recurrent parameters, let's stick with what we used for our RBC model
\bit
...
\item $\theta=2/3$ to match an average price duration of three quarters (Gal\'{i}-Lop\'{e}z-Salido EER 2001)

\item $\epsilon=6$ to match average markup of $20 \%$

\item $\phi = 1.5$ to match the Fed reaction function during the Greenspan era
\eit
...
\nu_t = \rho_{\nu} \nu_{t-1} + \epsilon_t \nonumber
\end{eqnarray}
with $\rho_{\nu} = 0.5$ to generate a ``moderately'' persistent shock, as we saw in the empirical IRFs
...
\begin{frame}{IRFs to a monetary policy shock}
...
\item This is because the model has no state variable, so dynamics are completely forward-looking:
\begin{eqnarray}
\text{DIS curve:} && \hat y_t = - (\hat i_t-E_t \pi_{t+1}) + E_t \hat y_{t+1} \nonumber \\
\text{Phillips curve:} && \pi_t = \beta E_t \pi_{t+1} + \kappa \hat y_t \nonumber \\
\text{Policy rule:} && \hat i_t = \phi \pi_t + \nu_t \nonumber 
...
\item Quantitatively, it is far off; we will get back to this in Lecture VI

\item To understand the mechanism, let's go back to the full 8-equation system, where we have cleaner interpretations of the equilibrium equations
\eit
...
\end{eqnarray}
where $\hat \omega_t = \hat w_t-p_t$ denotes log deviations in the real wage
...
\begin{frame}{IRFs to a monetary policy shock: full system}
...
\begin{frame}{IRFs to a monetary policy shock: mechanism}
...
\begin{frame}{IRFs to a monetary policy shock under flexible prices}
...
\item Zero response of all real variables, i.e., {\bc monetary neutrality}
...
\item Inflation falling in response to a ``contractionary'' monetary policy shock has nothing to do with sticky prices or other frictions; it is simply a consequence of a Taylor rule and rational expectations

\item The inflation response is so strong that the interest rate falls in response to a positive shock!
...
\bit
\setlength\itemsep{1em}
\item Sticky prices $\rightarrow$ smaller inflation response
...
\item {\bc Monetary non-neutrality!}
...
\bit
\item $\Rightarrow$ we may think of the output drop as being caused by a drop in {\bc aggregate demand}
\eit
...
\begin{frame}{A monetary policy shock is equivalent to a demand shock}
...
\item Naturally, we are interested in how sticky prices and the behavior of monetary policy affect the response to non-policy shocks
...
\item What role do sticky prices and monetary policy play for this response?
...
\item Let's instead consider a linearization around the {\bc flexible-price equilibrium} or, using another terminology, the {\bc natural equilibrium}
...
\ben
\setlength\itemsep{0.5em}
\item Monetary factors do not affect the natural real interest rate

\item Inefficient fluctuations are linked to deviations of the real interest rate from the natural real interest rate
\een
...
\begin{figure}
	...
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=.9\linewidth]{figures/wicksell_bok.jpg}
	\end{minipage}\hfill
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=.9\linewidth]{figures/wicksell.jpg}
	\end{minipage}
\end{figure}
...
\begin{center}
\footnotesize Source: Hillenbrand (RFS 2024)
\end{center}
...
\bit
\setlength\itemsep{1.5em}
\item A positive TFP shock leads to a ``gap'' recession

\item This reflects that monetary policy is not doing a good job

\item Mechanism: TFP $\uparrow$ $\Rightarrow$ marginal cost $\downarrow$ $\Rightarrow$ inflation $\downarrow$ $\Rightarrow$ interest rate $\downarrow$
\bit
\setlength\itemsep{0.5em}
...
\item This inefficiency is not surprising: the policy rule was specified in a completely ad hoc manner
...
\bit
\setlength\itemsep{1.5em}
\item TFP shocks are an example of a \emph{supply shock}
...
\item Because fluctuations in the natural equilibrium are efficient, the policy problem tends to be organized around how to undo the effect of price stickiness

\item Another class of supply shocks creates a wedge between the {\bc natural equilibrium} and the {\bc efficient equilibrium}, the so-called {\bc cost-push shocks}

\item Examples: shocks to firms' desired markups (greedflation?), shocks to distortionary taxes
\eit
...
\item Define $x_t = y_t-y^e_t$, where $y^e_t$ is the efficient equilibrium path.
...
\begin{frame}{IRFs to a cost-push shock}
...
\bit
	\item Here, a monetary authority can seemingly address two problems simultaneously by stimulating the economy
\eit
...
\item In contrast to RBC, NK predicts inefficient fluctuations and a role for monetary policy
