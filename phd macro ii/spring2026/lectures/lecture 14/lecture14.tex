...
\title[Course Summary]{\large Macroeconomics II, Lecture XIV:\\
...
\begin{itemize}
	...
	\item \textbf{Frictional labor markets:} McCall, Burdett-Mortensen, DMP
...
\begin{itemize}
	...
	\item Summarize the course material by putting these frameworks together in one model: a Heterogeneous-Agent New Keynesian model with search-and-matching frictions --- a \bf HANK-SAM \normalfont model
...
\begin{itemize}
	\setlength\itemsep{1.5em}
	\item \textbf{Heterogeneous-Agent New Keynesian} models: NK business-cycle
...
\begin{itemize}
	...
	\item Macro evidence: No (see, e.g., Yogo, REStat 2004; Canzoneri-Cumby-Diba,
	JME 2007)
...
to monetary policy, these responses are close-to-irrelevant to the representative (PIH) household

\item This is counterintuitive and at odds with what we know from the micro data
\begin{itemize}
	\item Recall lecture 12: mounting evidence of high MPCs
\end{itemize}
...
\begin{itemize}
	...
	\item HANK models replace the representative (PIH) household with a distribution of households that face limited insurance (as in Aiyagari) 
	\bit
		\setlength\itemsep{0.5em}
	...
	\item By so doing, they present a theory of aggregate demand that puts fluctuations in household income, wealth, credit, and risk at center stage
	...
	\bit
		...
		\item \bf Analytical frameworks: \normalfont Broer-Hansen-Krusell-{\"O}berg (REStud 2020); Acharya-Challe-Dogra (AER 2023); Bilbiie (REStud 2025)
		...
		\item \bf Evidence: \normalfont Cloyne-Ferreira-Surico (REStud 2020); Holm-Paul-Tischbirek (JPE 2021); Patterson (AER 2023) 
	\eit
...
\begin{itemize}
	...
	}HANK-SAM interaction raises the value of automatic stabilizers (especially
	unemployment insurance)
	\item \textbf{Challe (AEJmacro 2020):} HANK-SAM interaction changes optimal
	monetary policy
	\item \textbf{Broer-Druedahl-Harmenberg-{\"O}berg (2025):} A unified
	framework to evaluate the cost-effectiveness of different fiscal stabilization
	policies
\end{itemize}
\end{frame}

\begin{frame}{Broer-Druedahl-Harmenberg-{\"O}berg (2025)}
...
\begin{block}{Motivation}
	Resurgence of countercyclical fiscal policies as a stabilization tool
	\begin{itemize}
...
\begin{block}{Approach}
	Compute fiscal multipliers for different fiscal policies in an \bf HANK-SAM \normalfont model with an empirically grounded interaction between firms' hiring-and-firing decisions and households' consumption-saving decisions. 
\end{block}
...
\begin{itemize}
	...
	\begin{enumerate}
		...
		\begin{itemize}
			...
			\item Unemployed: receive UI benefits
		\end{itemize}
...
To first order, the equilibrium can be written as
\begin{align}
...
To first order, the solution is given by
\begin{equation}
...
\textbf{Implies:}
\begin{enumerate}
	\item GE amplification decomposes into three (HA-NK-SAM) blocks, admitting partial identification
	\item Direct effects are sufficient for policy comparison
	\item Relative multipliers of different policies are invariant to parameters outside the HA block 
\end{enumerate}
...
\centering%
\begin{tabular}{cc}
\includegraphics[width=0.44\linewidth]{results/policies_Y_titled} & \includegraphics[width=0.44\linewidth]{results/policies_tax_titled}\tabularnewline
\end{tabular}
\begin{itemize}
...
\begin{table}
	\begin{center}
		\input{results/fiscal_multipliers_tax.tex}  
	\end{center}
	\vspace{-2mm}
\end{table}
\vspace{1cm}
\begin{itemize}
	...
	\item Retention subsidies are most stimulative; universal transfers are least stimulative
\end{itemize}
\end{frame}
...
\input{results/fiscal_multipliers_all_pres_empty.tex}  
\end{small}
\vspace{-2mm}
\end{frame}
%
...
\begin{small}
\setlength\tabcolsep{2.5pt} 
\input{results/fiscal_multipliers_all_pres_H.tex}  
\end{small}
\begin{itemize}
...
\begin{small}
\setlength\tabcolsep{2.5pt} 
\input{results/fiscal_multipliers_all_pres_H_HA.tex}  
\end{small}
\begin{itemize}
...
frictions
\item Less nominal rigidity: subsidies are more effective relative to HH transfers/$G$
\end{itemize}
\end{frame}
%
...
\begin{itemize}
\item {\small Hiring subsidy is more effective with higher entry/lower separation
elasticity}{\small\par}
\item Lower MPC out of profits weakens both subsidies
\end{itemize}
\end{frame}
...
%	\setlength\itemsep{1em}	
%	
%	\item Exam 2023 May Question 2: First sub-question: Could you explain a bit more how we should arrive at this conclusion from the solutions: ``Given that the shock occurs in the firm optimality condition (3), let's take as given that inflation growth declines. This of course sounds reasonable to assume but I am a bit confused about what to assume and how to proceed. If we did not start with this assumption, can we still arrive at the right description of what happens in the problem?''
%	
...
%	
%	\item If you have found the optimality conditions for, for example, households and companies, why should you stop at restrictions and other conditions to find the solution in general equilibrium? This is done, among other things, in growth models, and in the Aiygari model?
%	
...
%	
%	\item Incomplete markets: basics, slide 32/42: if the credit constraint binds, MPC is high. Why is that? Is it because this implies that the marginal utility of consumption today is higher than the expected marginal value of assets tomorrow $V_m(M')$?
%	
%\end{itemize}
%\end{frame}
%
...
%	\item My reasoning:
%	\bit
%		\item firm optimality: holding $\hat mc_t$ constant, positive shock depresses inflation growth $\beta E_t \pi_{t+1}-\pi_t<0$. So let's assume $\beta E_t \pi_{t+1}-\pi_t<0$
%
...
%	\setlength\itemsep{1em}	
%	
%	\item For any model, the unconstrained social planner optimizes ``social welfare'' taking ``technological constraints'' as given
%	\bit
...
%	\setlength\itemsep{1em}	
%	
%	\item A constrained social planner typically refers to a planner that can set the decisions of all agents, but needs to respect the constraints each agent faces
%
%	
...
%	\setlength\itemsep{1em}	
%	
%	\item  \color{black} If you have found the optimality conditions for, for example, households and companies, why should you stop at restrictions and other conditions to find the solution in general equilibrium? This is done, among other things, in growth models, and in the Aiygari model?
%	
%\end{itemize}
%\end{frame}
%
...
%	
%	\item If not, we can often characterize the model analytically (N equations in N unknowns), as we do with the RBC or the NK model
%	
...
%	\eit  
%	
%	\item When it comes to answering questions on the exam, go as far as you need in terms of ``solving'' the model in order to adequately answer the question
%	
%\end{itemize}
%\end{frame}
%
...
%	
%	\item Asset demand comes from Firm F.O.C.
%	
...
%		
%		\item Search for symmetries: Is there a way to recast the problem so that it looks like a problem we've previously looked at
%		
%		\item Don't let algebraic mistakes stop you. If you have some intuition about how to get to the solution, write it out even if the equations are failing you
%	\eit
%	
%\end{itemize}
%\end{frame}
...
%
%\item Neither will you attract fewer workers if you post $x-\epsilon$
%
%\item Optimal wage posting strategy in such an environment just amounts to posting $w_{R}$
%
%\item In Burdett-Mortensen, workers have different reservation wages, because they have different wages, and search while employed
%
...
%\setlength\itemsep{1.5em}
%
%\item Expected profits are equalized in equilibrium, not actual profits
%
...
%
%\item We solved for a pure-strategy equilibrium, but a mixed strategy also works!
%
%\eit
%
%\end{frame}
...
%\bit
%\item per-worker profits decrease continuously
...
%	\setlength\itemsep{1em}	
%	
%	\item The first six equations solve for $\theta$
%	
